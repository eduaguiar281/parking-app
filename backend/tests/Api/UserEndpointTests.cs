...
public class UserEndpointTests : ApiTest
{
    ...
    [Fact]
    public async Task DeactivateUser_ultimoAdmin_retorna409()
    {
        var client = await AdminClient();
        var me = await client.GetFromJsonAsync<JsonElement>("/api/session");
        Assert.NotEqual(default, me.ValueKind);
        var id = me.GetProperty("id").GetGuid();
        var response = await client.PostAsync($"/api/users/{id}/deactivate", null);
