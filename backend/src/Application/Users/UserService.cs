...
public sealed class UserService(ParkingDbContext db, IPasswordHasher<User> hasher, AuditWriter audit)
{
    ...
    public async Task<IResult> UpdateAsync(Guid actorId, Guid id, UserWrite write)
    {
        ...
        var blocked = await RejectIfLastAdministratorAsync(user, write.Role, write.Status);
    ...
    public async Task<IResult> SetStatusAsync(Guid actorId, Guid id, UserStatus status)
    {
        ...
        var blocked = await RejectIfLastAdministratorAsync(user, user.Role, status);
        if (blocked is not null)
        {
            return blocked;
        }
    ...
    private async Task<IResult?> RejectIfLastAdministratorAsync(User user, UserRole nextRole, UserStatus nextStatus)
    {
        var activeAdmins = await db.Users.CountAsync(u =>
            u.Role == UserRole.Administrator && u.Status == UserStatus.Active);
        var isActiveAdmin = user.Role == UserRole.Administrator && user.Status == UserStatus.Active;
        var remainsActiveAdmin = nextRole == UserRole.Administrator && nextStatus == UserStatus.Active;
        if (!LastAdministrator.WouldLeaveNone(activeAdmins, isActiveAdmin, remainsActiveAdmin))
        {
            return null;
        }

        return HttpError.Json(409, "last_admin", "Não é permitido inativar ou rebaixar o último administrador ativo.");
    }
