...
5. **Entrada** — placa `abc1234` (sem hífen), carro, Pátio B, vaga sugerida; painel mostra ocupada e placa `ABC-1234`. Repetir a placa (recusa). Moto no Pátio B (recusa).
6. **Saída** — registrar saída com Pix; conferir resumo (primeira hora se < 1 h); vaga livre; receita no caixa; comprovante.
7. **Operador** — criar usuário operador, sair, entrar com ele: sem Vagas/Tarifas/Usuários/Relatórios; URL `/usuarios` recusada; fechar o caixa aberto pelo admin (permitido).
8. **Último admin** — com um só administrador ativo, tentar inativá-lo (recusa).
