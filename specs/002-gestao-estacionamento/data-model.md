...
- **Ciclo:** criado ativo ou inativo → inativar/reativar. Sem exclusão física nesta feature. NÃO inativar nem mudar para Operator o último Administrator ativo (409).
...
- **Ciclo:** Free → Occupied (entrada) → Free (saída ou cancelamento). Free → Blocked ou Maintenance (admin); Occupied não muda para Blocked/Maintenance. Não há transição Occupied → Occupied em outra vaga (sem troca de estadia ativa).
...
|-------|------|--------|
| id | uuid | PK |
| plate | string | maiúsculas; antigo `ABC-1234` (aceita entrada `ABC1234`); Mercosul `ABC1D23` |
| vehicleType | VehicleType | |
...
- **Identidade ativa:** no máximo uma estadia Active por placa; no máximo uma Active por vaga. Sem alteração de `spotId` enquanto Active.
...
- **Ciclo:** Open → Closed. Qualquer Operator ou Administrator fecha; `closedByUserId` é quem fechou. Reabrir (admin + motivo) → Open. Abrir nova data exige o Open atual Closed.
...
- Placa: normalizar maiúsculas; `ABC1234` (3 letras + 4 dígitos) → `ABC-1234`; aceitar só `ABC-1234` e `ABC1D23` depois da normalização.
...
Mensalista, reserva, segundo estabelecimento, integração Pix/adquirente, correção de estadia paga, troca de vaga com estadia ativa, segundo motor de banco, timeout de sessão por ociosidade.
