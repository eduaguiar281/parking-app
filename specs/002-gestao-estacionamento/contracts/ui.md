...
- Ação **Sair** no shell (encerra a sessão). Sem timeout por ociosidade: o painel permanece autenticado até Sair ou fechar o aplicativo.
...
Campos login e senha. Erro genérico se inválido ou inativo (sem revelar se o login existe). Sucesso: `/operacao` e cookie de sessão (não persistente).
...
Formulário de entrada: placa, tipo, setor (só compatíveis), vaga (manual ou “sugerir”). Aceitar `ABC1234` como `ABC-1234`. Sem vaga ou sem tarifa: mensagem que impede gravar. Sem ação de trocar vaga de estadia já ativa.
