...
Entregar o produto de gestão de um estacionamento sobre a fundação já existente: autenticação por cookie (Administrador e Operador), setores e vagas, tarifas parametrizáveis, entrada/saída com cobrança de hora cheia, um caixa aberto por vez, histórico, relatórios PDF/CSV, auditoria e UI pt-BR (R$, datas brasileiras) no visual do DESIGN.md.
...
**Constraints**: YAGNI > KISS > DRY > SOLID. Um csproj. Sem `IRepository<T>`, SignalR, Identity UI, segundo banco ou Pix/adquirente reais. Vitrine da fundação deixa de ser a home; `GET /alive` permanece. Código em inglês; docs/UI/nomes de teste em pt-BR
...
|-----------|--------|---------------------|
| I. YAGNI | Pass | Só módulos da spec. Sem mensalista, WebSocket, Identity completo, segundo DB |
| II. KISS | Pass | Minimal APIs, DbContext concreto, cookie + proxy, CSV manual |
| III. DRY | Pass | Preço, placa, esperado do caixa e ocupação em Domain; snapshots na estadia; DESIGN.md como fonte visual |
| IV. Clean Code | Pass | Identificadores em inglês; casos de uso nomeados; sem código morto da vitrine como home |
...
| VII. Testes | Pass | 80% nos READMEs; Domain cobre SC-003; API cobre 409 de vaga/placa/caixa; UI cobre bloqueio de rota do operador |
...
**Pós-Fase 1:** [data-model.md](./data-model.md) e contratos não introduzem entidade extra (comprovante e relatório são projeções). Gate de Ortogonalidade permanece Pass: regra de preço/placa/caixa esperado sem disco; persistência no contexto EF; nenhuma pasta D/A/I vazia. Tabela de complexidade vazia.
...
│   ├── Domain/                       # placa, preço, tarifa, ocupação, esperado
