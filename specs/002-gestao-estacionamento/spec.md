...
**Input**: User description: "Sistema Profissional de Gestão de Estacionamento (documento anexado): painel, setores e vagas, entrada, tarifas parametrizáveis, saída e pagamento, caixa diário, usuários e permissões, histórico, relatórios e auditoria, em pt-BR com valores em R$."

## Clarifications

### Session 2026-09-18

- Q: O sistema pode inativar ou rebaixar o último administrador ativo, deixando o estacionamento sem ninguém com acesso total? → A: Recusar. Tem de sobrar pelo menos um administrador ativo.
- Q: Com o veículo já estacionado, o operador pode trocar a vaga sem registrar saída e nova entrada? → A: Não. A estadia permanece na vaga da entrada até a saída ou até o administrador cancelar um lançamento ainda não pago.
- Q: Qualquer operador ou administrador pode fechar o caixa aberto, ou só quem o abriu? → A: Qualquer operador ou administrador fecha o caixa aberto; o sistema grava quem fechou.
- Q: Se o operador digitar a placa antiga sem hífen (`ABC1234`), o sistema aceita como o formato `ABC-1234`? → A: Aceitar. Normalizar `ABC1234` para `ABC-1234` (letras maiúsculas). Mercosul (`ABC1D23`) continua sem hífen.
- Q: Depois do login, a sessão do operador permanece até ele sair, ou expira sozinha se ficar um tempo sem usar o sistema? → A: A sessão vale até o logout ou o fechamento do aplicativo.
...
**Independent Test**: Criar um administrador e um operador, entrar com cada um e confirmar menus e bloqueios; tentar entrar com usuário inativo e senha errada; sair da sessão e confirmar que precisa autenticar de novo.
...
5. **Given** um usuário autenticado ocioso no painel, **When** o tempo passa sem ele usar o sistema, **Then** a sessão permanece válida até ele sair ou fechar o aplicativo.
6. **Given** um usuário autenticado, **When** escolhe sair ou fecha o aplicativo, **Then** a sessão encerra e a próxima abertura exige login.
...
**Independent Test**: Abrir o caixa do dia com um operador, registrar uma sangria e um suprimento (como administrador), fechar o caixa com outro operador conferindo totais e diferença, e tentar abrir um segundo caixa enquanto o primeiro ainda está aberto.
...
4. **Given** um caixa aberto por um operador, **When** outro operador ou um administrador informa o valor de fechamento e observações, **Then** o caixa fecha: o sistema grava esperado (abertura + receitas de saídas + suprimentos − sangrias ± ajustes), informado, diferença, data, hora e o responsável pelo fechamento (quem fechou, não necessariamente quem abriu), e impede novos pagamentos nesse caixa.
...
5. **Given** placa em minúsculas, com hífen, sem hífen no formato antigo, ou Mercosul válido, **When** registra a entrada, **Then** a placa é gravada em maiúsculas: antigo como `ABC-1234` (aceitando `ABC1234`) e Mercosul como `ABC1D23`.
...
7. **Given** um veículo com estadia ativa, **When** alguém tenta apenas trocar a vaga sem saída, **Then** a troca é recusada; a vaga original permanece ocupada.
...
**Independent Test**: Criar um operador, inativá-lo, confirmar que não entra, reativá-lo, confirmar que um operador não acessa essa tela, e tentar inativar o último administrador ativo.
...
2. **Given** um usuário existente que não é o último administrador ativo, **When** o administrador o inativa, **Then** esse usuário deixa de entrar; ao reativar, volta a entrar.
3. **Given** um operador autenticado, **When** tenta criar, editar, inativar ou reativar usuários, **Then** a ação é recusada.
4. **Given** um único administrador ativo restante, **When** alguém tenta inativá-lo ou mudar seu perfil para Operador, **Then** a ação é recusada com mensagem clara e o administrador permanece ativo.
...
- Placa inválida (fora dos formatos brasileiro antigo e Mercosul): a entrada é recusada antes de ocupar vaga. Placa antiga sem hífen (`ABC1234`) é válida e gravada como `ABC-1234`.
...
- Tentativa de trocar a vaga de um veículo já estacionado sem saída: recusada; a estadia permanece na vaga original.
...
- Fechamento do caixa aberto por um usuário diferente de quem abriu: permitido para qualquer operador ou administrador; quem fecha é o responsável pelo fechamento.
...
- Sessão ociosa: permanece autenticada até o logout ou o fechamento do aplicativo; não cai sozinha por inatividade.
- Tentativa de inativar ou rebaixar o último administrador ativo: recusada; o usuário permanece administrador ativo.
...
- **FR-009**: O sistema DEVE autenticar por login (usuário ou e-mail) e senha. A senha NÃO DEVE ser recuperável em texto claro. A sessão DEVE permanecer válida até o usuário sair explicitamente ou fechar o aplicativo; NÃO DEVE expirar só por tempo ocioso. DEVE existir a ação de sair.
...
- **FR-014**: Apenas administradores DEVEM criar, editar, inativar ou reativar usuários. NÃO DEVE ser permitido inativar nem mudar para Operador o perfil do último administrador ativo: a ação DEVE ser recusada com mensagem clara e esse usuário DEVE permanecer administrador ativo.
...
- **FR-034**: Placas DEVEM ser normalizadas para maiúsculas. O formato antigo DEVE aceitar `ABC-1234` e `ABC1234`, gravando sempre `ABC-1234`. O formato Mercosul DEVE ser `ABC1D23` (sem hífen). Busca por placa DEVE encontrar o veículo independentemente de o operador ter digitado o hífen.
...
Termo canônico na spec e na interface: **estadia** (ciclo de um veículo no pátio; às vezes chamado atendimento).
...
- A data operacional do caixa é a data de calendário informada na abertura. Só um caixa permanece aberto por vez em todo o sistema; o dia anterior precisa ser fechado antes de abrir o seguinte. Qualquer operador ou administrador pode fechar o caixa aberto; o registro de fechamento aponta quem fechou.
...
- O administrador inicial existe para o primeiro acesso (criado na implantação). Não há auto cadastro público. Sempre resta pelo menos um administrador ativo; inativar ou rebaixar o último é recusado. A sessão dura até o logout ou o fechamento do aplicativo; não há timeout por ociosidade nesta feature.
...
- Na sugestão automática, o sistema escolhe uma vaga livre compatível qualquer; o operador pode trocar antes de confirmar. Depois da entrada confirmada, não há troca de vaga: a ocupação só se encerra na saída ou no cancelamento administrativo de estadia ativa não paga. Reserva temporária enquanto o formulário está aberto está fora desta feature.
- Tarifa geral (sem setor) e tarifa de setor do mesmo tipo podem coexistir ativas: na entrada, a do setor tem precedência.
- Formato antigo de placa aceita `ABC-1234` e `ABC1234`, sempre gravado com hífen. Mercosul é `ABC1D23`. Letras vão para maiúsculas. A busca ignora o hífen na digitação.
...
- Troca de vaga de veículo já estacionado sem registrar saída.
